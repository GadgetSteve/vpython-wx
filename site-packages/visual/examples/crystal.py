#!/usr/bin/env python
#coding:utf-8
"""
Demostration of bonds in crystal lattis
"""
# Bruce Sherwood; revised by Jonathan Brandmeyer
# Converted core calculations to numpy, Sept. 2010, Bruce Sherwood

from __future__ import print_function, division
from random import random
import visual as vp

PI = vp.pi

INDEX = 4
N_TOTAL = INDEX*INDEX*INDEX
SCOLOR = (1, 0.5, 0)
BCOLOR = (0, 0.58, 0.69)
SPRINGS = []
ATOM = []
M = 1.
K = 1.
L = 1.
R = 0.3*L
RS = 0.9*R # end of spring is Rs from center of atom

def getn(n_val, n_x, n_y, n_z):
    """ find nth atom given nx, ny, nz"""
    return (n_y)*(n_val**2)+(n_x)*n_val+(n_z)

def makespring(natom1, natom2, radius):
    """ make spring from nnth atom to iith atom"""
    if natom1 > natom2:
        r12 = ATOM[natom2].pos-ATOM[natom1].pos
        direct = vp.norm(r12)
        SPRINGS.append(vp.helix(pos=ATOM[natom1].pos+RS*direct,
                                axis=(L-2*RS)*direct,
                                radius=radius,
                                color=SCOLOR, thickness=0.04)) #, shininess=0.9))
        SPRINGS[-1].atom1 = ATOM[natom1]
        SPRINGS[-1].atom2 = ATOM[natom2]
        angle = SPRINGS[-1].axis.diff_angle(vp.vector(0, 1, 0))
        # avoid pathologies if too near the y axis (default "up")
        if angle < 0.1 or angle > PI-0.1:
            SPRINGS[-1].up = vp.vector(-1, 0, 0)

def crystal(index=3, delta=1.0, radius=None, sradius=None):
    """ Create the crystal."""
    if radius is None:
        radius = 0.2*delta
    if sradius is None:
        sradius = radius/5.
    xmin = -(INDEX-1.0)/2.
    ymin = xmin
    zmin = xmin
    natom = 0
    for n_y in range(INDEX):
        yval = ymin+n_y*delta
        hue = (n_y)/(INDEX+1.0)
        col = vp.color.hsv_to_rgb((hue, 1.0, 1.0))
        for n_x in range(INDEX):
            x_val = xmin+n_x*delta
            for n_z in range(INDEX):
                z_val = zmin+n_z*delta
                ATOM.append(vp.sphere(pos=(x_val, yval, z_val), radius=radius,
                                   color=BCOLOR))
                ATOM[-1].p = vp.vector()
                ATOM[-1].near = list(range(6))
                ATOM[-1].wallpos = list(range(6))
                ATOM[-1].natom = natom
                ATOM[-1].indices = (n_x, n_y, n_z)
                natom = natom+1
    for atm in ATOM:
        natom1 = atm.natom
        n_x, n_y, n_z = atm.indices
        if n_x == 0: # left
            # if this neighbor is the wall, save location:
            atm.near[0] = None
            atm.wallpos[0] = atm.pos- vp.vector(L, 0, 0)
        else:
            natom2 = getn(INDEX, n_x-1, n_y, n_z)
            atm.near[0] = natom2
            makespring(natom1, natom2, sradius)
        if n_x == INDEX-1: # right
            atm.near[1] = None
            atm.wallpos[1] = atm.pos+ vp.vector(L, 0, 0)
        else:
            natom2 = getn(INDEX, n_x+1, n_y, n_z)
            atm.near[1] = natom2
            makespring(natom1, natom2, sradius)

        if n_y == 0: # down
            atm.near[2] = None
            atm.wallpos[2] = atm.pos- vp.vector(0, L, 0)
        else:
            natom2 = getn(INDEX, n_x, n_y-1, n_z)
            atm.near[2] = natom2
            makespring(natom1, natom2, sradius)
        if n_y == INDEX-1: # up
            atm.near[3] = None
            atm.wallpos[3] = atm.pos+ vp.vector(0, L, 0)
        else:
            natom2 = getn(INDEX, n_x, n_y+1, n_z)
            atm.near[3] = natom2
            makespring(natom1, natom2, sradius)

        if n_z == 0: # back
            atm.near[4] = None
            atm.wallpos[4] = atm.pos- vp.vector(0, 0, L)
        else:
            natom2 = getn(INDEX, n_x, n_y, n_z-1)
            atm.near[4] = natom2
            makespring(natom1, natom2, sradius)
        if n_z == INDEX-1: # front
            atm.near[5] = None
            atm.wallpos[5] = atm.pos+ vp.vector(0, 0, L)
        else:
            natom2 = getn(INDEX, n_x, n_y, n_z+1)
            atm.near[5] = natom2
            makespring(natom1, natom2, sradius)
        atm.near = tuple(atm.near)
        atm.wallpos = tuple(atm.wallpos)
        # Nearpos is a list of references to the nearest neighbors' positions,
        # taking into account wall effects.
        atm.nearpos = []
        for i in range(6):
            natom = atm.near[i]
            if natom is None: # if this nearest neighbor is the wall
                atm.nearpos.append(atm.wallpos[i])
            else:
                atm.nearpos.append(ATOM[natom].pos)

    return ATOM

SRADIUS = R/3.
VRANGE = 0.2*L* vp.sqrt(K/M)
DT = 0.02*(2.*PI* vp.sqrt(M/K))
ATOM = crystal(index=INDEX, delta=L, radius=R, sradius=SRADIUS)
vp.scene.autoscale = False

PTOTAL = vp.vector()
for a in ATOM:
    px = M*(-VRANGE/2+VRANGE*random())
    py = M*(-VRANGE/2+VRANGE*random())
    pz = M*(-VRANGE/2+VRANGE*random())
    a.p = vp.vector(px, py, pz)
    PTOTAL = PTOTAL+a.p

for a in ATOM:
    a.p = vp.array(a.p-PTOTAL/(INDEX**2))

# Convert to tuples for faster indexing access.  We aren't growing any more of them.
SPRINGS = tuple(SPRINGS)
ATOM = tuple(ATOM)

# Evaluate a couple of constants outside the loop
K_DT = K * DT
DT_M = DT / M

while True:
    vp.rate(100)
    for a in ATOM:
        r = vp.array(a.nearpos) - a.pos
        rmag = (vp.sqrt(vp.sum(vp.square(r), -1))).reshape(-1, 1) # reshape rmag from row to column
        a.p += K_DT * vp.sum((1-L/rmag)*r, 0) # sum the forces k*dt*(rmag-L)*(r/rmag)

    for a in ATOM:
        a.pos += a.p * DT_M

    for s in SPRINGS:
        p1 = s.atom1.pos
        r12 = s.atom2.pos-p1
        direction = r12.norm()
        s.pos = p1+RS*direction
        s.axis = (r12.mag-2*RS)*direction

