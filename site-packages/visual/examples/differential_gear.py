...
#from visual import (rate, scene, frame, shapes, extrusion, paths, pi, color,
                    #box, cylinder, vector, materials, curve, label)
import visual as vp
PI = vp.pi

print(__doc__)
vp.scene.width = vp.scene.height = 800
vp.scene.forward = (0.2, -0.6, -0.8)
vp.scene.title = "Differential Gear"
...
P1 = vp.box(frame=EG6.frame, pos=(R6/2, 0, R5), size=(1, R5, R5*2),
            color=(0, 0, 1), opacity=0.5)
P2 = vp.box(frame=EG6.frame, pos=(-R6/2., 0, R5), size=(1, R5, R5*2),
            color=(0, 0, 1), opacity=0.5)
SHFT = vp.cylinder(frame=EG6.frame, pos=P1.pos, axis=P2.pos-P1.pos, radius=0.5,
                   color=(0, 1, 0))
"""
...
SC = vp.extrusion(shape=vp.shapes.rectangle(width=R6+1, height=R5)-vp.shapes.circle(radius=R2/3),
                  ...
                  material=vp.materials.glass)


CC = vp.curve(pos=[(-25, 0, R6), (+25, 0, R6)])
RUN = False
ANG = PI/200
ANG2 = ANG/(R6/R5)
STRANG = ANG2/2.0
DFLAB = "            CLICK TO START OR PAUSE\nPRESS  R  OR  L  TO TURN, S FOR STRAIGHT"
LBL = vp.label(yoffset=300, line=0,
            text=DFLAB)
KEY = "s"
CAPTION_DICT = {
    "r":"TURNING RIGHT",
    "l":"TURNING LEFT",
    "s":"DRIVING STRAIGHT - PRESS  R  OR  L  TO TURN",
}

def do_subrotate(dirkey):
    """ Do the subframe rotations."""
    dir_dict = {'l':-1, 'r':1}
    dirv = dir_dict.get(dirkey)
    if not dirv is None:
        EG2.frame.rotate(axis=(0, 0, 1), angle=-STRANG*dirv, origin=F2.pos)
        EG4.frame.rotate(axis=(0, 0, 1), angle=+STRANG*dirv, origin=F4.pos)
        EG1.frame.rotate(axis=(1, 0, 0), angle=-STRANG*dirv, origin=F1.pos)
        EG3.frame.rotate(axis=(1, 0, 0), angle=+STRANG*dirv, origin=F3.pos)

while True:
    if vp.scene.mouse.events:
        M_EVT = vp.scene.mouse.getevent()
        if M_EVT.click == 'left':
            RUN = not RUN
    if vp.scene.kb.keys:
        KEY = vp.scene.kb.getkey().lower()
        LBL.text = CAPTION_DICT.get(KEY, DFLAB)
        if KEY == ' ':
            RUN = not RUN
    if RUN:
